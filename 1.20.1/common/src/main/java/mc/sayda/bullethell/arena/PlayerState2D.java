...
public class PlayerState2D {
    ...
    /**
     * Counts down the bomb cut-in. The bomb is committed the moment it is pressed
     * (stock spent, invulnerability on, pending death cancelled) but its effect -
     * clearing bullets, failing the spell, vacuuming items - is held until this
     * reaches zero, so the portrait lands before the screen wipes.
     * {@code 0} = no bomb in flight.
     */
    public int bombStartupTicks = 0;
    ...
    /**
     * Power every participant starts an arena with. Set high because the pre-boss
     * fairy waves that used to supply power items were removed; boss phase
     * transitions top players back up from here.
     */
    ...
    public PlayerState2D() {
        this(HIT_RADIUS, GRAZE_RADIUS, PICKUP_RADIUS, SPEED_NORMAL, SPEED_FOCUSED, 3, 3);
    }
    ...
    public void move(float dx, float dy) {
        float speed = focused ? speedFocused : speedNormal;
        if (dx != 0 && dy != 0) {
