...
public class PlayerState2D {
    ...
    public int powerLevel() {
        if (power >= MAX_POWER) return 4;
        if (power >= 96)  return 3;
        if (power >= 32)  return 2;
        if (power >= 8)   return 1;
        return 0;
    }
    ...
    /** Character-aware constructor called from ArenaContext with CharacterDefinition values. */
    ...
    /** Reset to spawn position after death. */
    public void respawn() {
        x = BulletPool.ARENA_W / 2f;
        y = BulletPool.ARENA_H * 0.85f;
        deathPendingTicks = 0;
