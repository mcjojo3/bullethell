...
public class LaserPool {
     ...
     * 
     * @param bidirectional true for NDL-style beams that extend in both directions
     *                      through the origin;
     *                      false for directional beams (Master Spark) that only
     *                      extend forward.
     */
    ...
    public boolean isWarning(int i) {
        return active[i] && data[i * STRIDE + F_WARN] > 0;
    }

    public boolean isFiring(int i) {
        return active[i] && data[i * STRIDE + F_WARN] <= 0;
    }

    public boolean isBidir(int i) {
        return bidir[i];
    }

    public float getX(int i) {
        return data[i * STRIDE + F_X];
    }

    public float getY(int i) {
        return data[i * STRIDE + F_Y];
    }

    public float getAngle(int i) {
        return data[i * STRIDE + F_ANGLE];
    }

    public float getHalfWidth(int i) {
        return data[i * STRIDE + F_HW];
    }

    public int getWarnLeft(int i) {
        return (int) data[i * STRIDE + F_WARN];
    }

    public int getActLeft(int i) {
        return (int) data[i * STRIDE + F_ACT];
    }

    public int getTypeId(int i) {
        return (int) data[i * STRIDE + F_TYPE];
    }
