 ...
 * Per-difficulty values use the same order as {@code spellDurationTicks}: {@code [EASY, NORMAL, HARD, LUNATIC]}.
 * You may either (1) put that array <strong>on the scalar key</strong> (e.g. {@code "speed": [2.2, 2.4, 2.6, 2.9]} - load
 * copies it to the internal tier field and sets the scalar to the first entry), (2) use a {@code byDifficulty}
 * object whose keys match scalar names, or (3) legacy separate {@code *ByDifficulty} keys. When the same field
...
public class PatternStep {
     ...
     * PENTAGRAM_RITUAL (stacked pentagrams → ring → true-star edge comb; optional scripted follow-up or JSON {@code ORB_C_ROW}),
     * ORB_C_ROW (curved orb row in one direction; pentagram-ritual phase-4 style, JSON-driven),
     * STACK_FAN_VOLLEY (random annulus origin around boss: 3 stacked-ray arms in a downward fan of {@code bulletType}
     * + 1 stacked ray of {@code ringBulletType} aimed at player; legacy alias {@code DAGGER_HALO_VOLLEY}),
     * SEA_SPLIT (scripted curtain + secondaries in {@code ArenaContext}),
     * DIVINE_WIND (Yasaka custom spiral placement + C-turn release; scripted in {@code ArenaContext}),
     * RAIN, BOUNCE, MEISTER_CYCLE (Remilia Scarlet Meister - scripted in {@code ArenaContext})
     */
    public String pattern = "RING";

    /**
     * Ticks after this step fires before the boss attack rotation advances (firerate for that slot).
     * Not bullet travel speed - that is {@link #speed}. Effective gap is at least {@link #minCooldown}
     * after difficulty scaling (see {@code ArenaContext.computeAttackCooldown}); {@code 0} = as fast as the tick loop allows.
     */
    ...
    /**
     * Bullet <em>travel</em> speed (arena units per tick, before difficulty scaling): velocity magnitude
     * passed into {@link mc.sayda.bullethell.pattern.PatternEngine} spawns. It does <strong>not</strong> change
     * how often the boss fires this step - that is {@link #cooldown} and {@link #minCooldown} (server
     * volley gap each cycle). For {@code SPRINKLER}, emitter rotation per shot is
     * {@link #sprinklerAdvanceRad} per shot; to spray faster, lower {@link #cooldown} / {@link #minCooldown}, not {@code speed}.
     */
    ...
    // ---- Optional per-difficulty scalars [EASY, NORMAL, HARD, LUNATIC] (see DifficultyTierArray) ----

    public int[] armsByDifficulty = null;
    public int[] cooldownByDifficulty = null;
    public int[] minCooldownByDifficulty = null;
    public int[] maxScaledArmsByDifficulty = null;
    public int[] burstCountByDifficulty = null;
    public int[] burstIntervalByDifficulty = null;
    public int[] bulletLifetimeTicksByDifficulty = null;
    public int[] segmentDurationTicksByDifficulty = null;
    public int[] segmentVolleyIntervalTicksByDifficulty = null;
    public int[] rayStackDepthByDifficulty = null;
    public float[] speedByDifficulty = null;
    public float[] spreadByDifficulty = null;
    public float[] rayStackSpacingByDifficulty = null;
    public float[] laserHalfWidthByDifficulty = null;
    public float[] laserRotateAdvanceRadByDifficulty = null;
    public float[] sprinklerAdvanceRadByDifficulty = null;
    public int[] combCountByDifficulty = null;
    public float[] orbCRowSpacingScaleByDifficulty = null;
    public float[] orbCRowCurvatureScaleByDifficulty = null;
    public float[] orbCRowSpeedSlopeByDifficulty = null;

    /**
     * Generic per-difficulty overrides: keys match scalar JSON field names; each value is
     * {@code [EASY, NORMAL, HARD, LUNATIC]}. Optional. Legacy {@code *ByDifficulty} fields take precedence.
     */
     ...
     * 0 = profile default (profiles default to {@code 1} = off unless you set this).
     */
    public float pressureSoftCap = 0f;
    /** Optional max fractional arm reduction under full pressure (0..1). 0 = profile default. */
    ...
    /**
     * Minimum volley gap (ticks) after difficulty scaling. {@code 0} = no extra floor beyond scaled {@link #cooldown}.
    ...
    /**
     * {@code PENTAGRAM_RITUAL} only: if &gt; 0, phase-4 follow-up lasts this many ticks (aimed C-rows),
     * then optionally {@link #pentagramLoopRitual} restarts the ritual. 0 = legacy follow-up until
     * the phase ends (wider random-heading rows).
     */
    public int pentagramFollowupDurationTicks = 0;

    /**
     * {@code PENTAGRAM_RITUAL} only: after phase-4 follow-up duration (when used), restart from phase 1.
     * If {@link #skipPentagramRitualFollowup} is true, restarts immediately after edge combs instead of orb volleys.
     */
    public boolean pentagramLoopRitual = false;

    /**
     * {@code PENTAGRAM_RITUAL} only: when true, scripted phase 4 (orb C-rows) is <strong>disabled</strong>.
     * After the edge-comb wave, either normal {@code attacks} rotation runs (other steps in the phase), or
     * if {@link #pentagramLoopRitual} is also true the pentagram cycle restarts with no orb volleys.
     */
    public boolean skipPentagramRitualFollowup = false;

    /**
     * {@code PENTAGRAM_RITUAL} only: draw paired outline bullets per sample - {@link #bulletType} (inner ring)
     * and {@link #ringBulletType} (outer ring). Both start stacked on the boss; outer ring eases to a larger
     * radius than inner (see {@link #pentagramInnerRingScale} / {@link #pentagramOuterRingScale}).
     */
    public boolean pentagramDualOverlapped = false;

    /**
     * {@code PENTAGRAM_RITUAL} dual mode: multiplier on the eased ring-out radius for {@link #bulletType}
     * (inner ring). Ignored when {@link #pentagramDualOverlapped} is false.
     */
    public float pentagramInnerRingScale = 1f;

    /**
     * {@code PENTAGRAM_RITUAL} dual mode: max ring radius multiplier for {@link #ringBulletType}.
     */
    public float pentagramOuterRingScale = 1f;

    /**
     * {@code PENTAGRAM_RITUAL} only: minimum boss-AI ticks after the last {@code beginNewWave} before another
     * stack may start, once the current spawn wave has finished stacking. {@code 0} = single stack. Requires
     * {@link #pentagramSkipEdgeComb} if edge combs must not clear older outline waves.
     */
    public int pentagramRepeatStackTicks = 0;

    /**
     * {@code PENTAGRAM_RITUAL} only: skip per-star edge-comb dissolution (keeps outline bullets kinematic until
     * lifetime). Set true for dual looping spells.
     */
    public boolean pentagramSkipEdgeComb = false;

    /**
     * {@code ORB_C_ROW} only: curvature scale for {@link mc.sayda.bullethell.pattern.PatternEngine#fireOrbCRowInDirection}
     * ({@code <= 0} = same tiered default as pentagram ritual phase 4).
     */
    public float orbCRowCurvatureScale = 0f;

    /**
     * {@code ORB_C_ROW} only: row spacing tightness ({@code ~0.58} = ritual default); {@code <= 0} = use ritual default.
     */
    public float orbCRowSpacingScale = 0f;

    /**
     * {@code ORB_C_ROW} only: when true, each volley picks a random flight angle instead of aiming at
     * the nearest player.
     */
    ...
    /**
     * {@code STACK_FAN_VOLLEY} only: bullets per ray, laid out along flight direction from the spawn
     * (first bullet at the halo sample, rest farther along velocity); {@code <= 0} = 10.
     */
    public int rayStackDepth = 0;

    /**
     * {@code STACK_FAN_VOLLEY} only: spacing between consecutive bullets along the ray (arena units);
     * {@code <= 0} = 3.
     */
    ...
    /**
     * When true, {@code SEA_SPLIT} secondary volleys sample each burst origin on a random annulus around
     * the boss (same geometry as {@code PENTAGRAM_RITUAL} phase-4 halo). Ignored for primaries and
     * non-{@code SEA_SPLIT} contexts.
     */
    public boolean fireFromRandomBossHalo = false;

    /** Inner radius for {@link #fireFromRandomBossHalo}; {@code <= 0} uses ritual default ({@code 18}). */
    public float randomHaloMinR = 0f;

    /** Outer radius; {@code <=} inner uses ritual default ({@code 98}). */
    public float randomHaloMaxR = 0f;

    /** Extra ± jitter on X/Y after polar sample; {@code <= 0} uses ritual default ({@code 26}). */
    ...
    /**
     * When true, this step is <strong>not</strong> part of the main attack rotation. It runs once
     * every boss AI tick while its phase is active (same timing as movement), independent of
     * {@link #cooldown} on other steps. Use for continuous {@code SPRINKLER} / streams while slower
     * patterns (e.g. {@code AIMED}) advance the normal {@code attacks} cycle.
     */
    public boolean everyTickWhilePhase = false;

    /**
     * When &gt; 0, this step holds the attack rotation for exactly this many boss AI ticks (wall-clock).
     * Volleys during that window are spaced by {@link #segmentVolleyIntervalTicks}. When time runs out,
     * the rotation advances to the next attack. {@code 0} = normal rotation / burst behaviour.
     * Do not combine with {@link #everyTickWhilePhase} on the same step (every-tick path skips when this &gt; 0).
     */
    public int segmentDurationTicks = 0;

    /**
     * Minimum ticks between volleys while a {@link #segmentDurationTicks} segment is active.
     * {@code 1} = fire every tick (subject to segment duration). {@code 0} or omitted = treat as {@code 1}.
     */
    public int segmentVolleyIntervalTicks = 1;

    /**
     * Optional segment sync group id. When non-empty on a segmented step, contiguous steps in the
     * same phase attack list with the same id fire together on each segment volley and advance together
     * when the segment ends. Empty = normal one-step segment behavior.
     */
    public String segmentSyncGroup = "";

    /**
     * {@code SPRINKLER} only: radians the emitter rotates between consecutive shots (scaled by the
     * phase's {@link mc.sayda.bullethell.boss.PhaseDefinition#patternTempo}). Positive = clockwise,
    ...
    /**
     * {@code SPRINKLER} comb mode: when &gt; 0, each nozzle (see {@link #arms}) fires a fan of
     * {@link #combCount} bullets instead of a single bullet. Value = radians between adjacent
    ...
    /**
     * {@code SPRINKLER} ring mode only: spawn one arm per volley in order around the ring. Each bullet stays
     * frozen (no movement, no lifetime decay) until every arm in the ring has fired; freeze length for arm
     * {@code i} is {@code (arms - 1 - i) × sprinklerSpawnStaggerTicks}. Use with {@link #segmentVolleyIntervalTicks}
     * to space spawns. After release, bullets use normal outward velocity and {@link #bulletAngularVelocity}.
     */
    public boolean sprinklerSequentialRing = false;

    /**
     * Extra freeze stagger per arm index for {@link #sprinklerSequentialRing} (ticks). {@code 0} = no group hold
     * (still one bullet per volley).
     */
    public int sprinklerSpawnStaggerTicks = 1;

    /**
     * For {@link #sprinklerSequentialRing}: max radial offset from the boss along each arm while placing the
     * spiral (0 = all bullets spawn at the boss origin).
     */
    public float sprinklerSpawnRadiusMax = 0f;

    /**
     * {@code DIVINE_WIND} only: number of ring layers to place before layer index wraps.
     * Typical usage is 21 so each arm receives 21 placed bullets per cycle.
     */
    public int divineWindLayers = 21;

    /**
     * {@code DIVINE_WIND} only: radial spacing (arena units) between consecutive placed layers.
     */
    public float divineWindLayerSpacing = 8.0f;

    /**
     * {@code DIVINE_WIND} only: freeze stagger per layer in ticks while building the pre-release stack.
     * Freeze for layer {@code i} is {@code (layers - 1 - i) × divineWindFreezeStaggerTicks}.
     */
    public int divineWindFreezeStaggerTicks = 1;

    /**
     * {@code DIVINE_WIND} only: tangential velocity factor relative to {@link #speed} after release.
     */
    public float divineWindTangentialFactor = 0.90f;

    /**
     * {@code DIVINE_WIND} only: inward radial velocity factor relative to {@link #speed} after release.
     * Positive values push slightly inward first (a C-like fold) before the angular turn swings outward.
     */
    public float divineWindInwardFactor = 0.22f;

    /**
     * {@code DIVINE_WIND} only: additional angular velocity (radians/tick) applied to released bullets.
     * Sign should usually oppose {@link #sprinklerAdvanceRad} to produce the characteristic C-shaped turn.
     */
    public float divineWindCurveAngularVelocity = 0.03f;

    /**
     * {@code DIVINE_WIND} only: moving ticks to keep the curved C-turn active after release, then bullets
     * are forced to continue straight away from the boss.
     */
    public int divineWindCurveTicks = 22;

    /**
     * {@code PENTAGRAM_RITUAL} dual release only: angular velocity for inner set ({@link #bulletType})
     * after split/disassembly. Use non-zero for curving red while blue remains straight.
     */
    public float pentagramDualInnerReleaseAngularVelocity = 0f;

    /**
     * {@code PENTAGRAM_RITUAL} dual release only: angular velocity for outer set ({@link #ringBulletType}).
     */
    public float pentagramDualOuterReleaseAngularVelocity = 0f;

    /**
     * {@code PENTAGRAM_RITUAL} dual release only: total bullets per inner outline bullet after disassembly.
     * {@code 1} = no split, {@code 2} = one extra clone, etc. Clamped in runtime.
     */
    public int pentagramDualInnerSplitCount = 1;

    /**
     * {@code PENTAGRAM_RITUAL} dual release only: radians between adjacent inner split lanes.
     * Used when {@link #pentagramDualInnerSplitCount} &gt; 1.
     */
    public float pentagramDualInnerSplitSpreadRad = 0f;

    /**
     * {@code PENTAGRAM_RITUAL} dual release only: speed multiplier for cloned inner split bullets.
     */
    public float pentagramDualInnerSplitSpeedMul = 1f;

    /**
     * {@code SPRINKLER} comb mode: bullets per nozzle when {@link #sprinklerSpread} &gt; 0.
     * {@link #arms} controls how many evenly-spaced nozzles fire simultaneously; this controls
     * how many bullets each nozzle fires in a fan. Default 1 (single bullet per nozzle).
     */
    public int combCount = 1;

    /**
     * {@code SPRINKLER} comb mode only: per-index speed slope across the comb fan.
     * Speed multiplier for bullet {@code i} is {@code 1 + (i - center) * sprinklerSpeedSlope}.
     * Example: {@code 0.1} makes one side of the comb travel farther/faster, creating a slanted lane front.
     */
